...
An accelerometer consists mainly of flexible mounted mass, which is under the influence of a force.
...
Gyroscopes are an inertial sensor, which detect the change of position.
Historically they were flywheels mounted in a Cardan suspension, based on the principles of conservation of angular momentum.
Once spinned up they remain at their orientation.
...
The carrier medium for the waves is air, 

When mounted on mobile systems, 
...
At the time of the search~(2008), the fastest gaming mice had a maximum speed of 1\,m/s.\\
...
% - max. 1mps
% - due to constructed as an exact gaming device, they have a very short working range of only 2\,mm to allow lifting off and relocating the mouse very fast.

Laser mice use the same principle as the before mentioned laser speckle sensor.
The only difference to normal mice is the use of a coherent laser source as illumination instead of a LED.
...
The direction of the fibre results in globally different friction factors in every dimension.
...
The rotary encoders are mounted on the motors and not on the wheel.
When the connection between motor and the wheel got loose, the rotary encoders delivered only the rotation of the motor and not the distance travelled. 
This resulted in complete disorientation of the localisation software on an overrunning motor.\\
But faulty mechanics is not always responsible for this type of error, it is sufficient if a wheel looses contact with the ground for a moment.
...
Therefore data quality information would be highly appreciated on every sensor used.
...
But the big disadvantages of industrial sensors are the their size, weight and power requirement.
...
At last, the components involving the camera and the FPGA processor would be unaffordable for even one robot.
