...
\begin{itemize}
 ...
 \item Single 3.3 volt power supply
...
It comes in a 20-pin package, bundled with its own lens and matching VCSEL~(Vertical-cavity surface-emitting laser) laser diode.
It uses 3.3V, but can be connected to a 5\,V-microcontroller without level shifting techniques needed.
Some circuit is needed, as shown in~\autoref{fig:adnsc}.
...
The ADNS-6010 is mounted on a custom PCB (Printed circuit board).
This board contains all needed circuit according to the data sheet, and a 10-pin header for connection to the microcontroller board.
The board layout is designed in the Cadsoft EAGLE Software.
...
A 10-pin header was soldered on the microcontroller board and connected to the needed pins of the XC~core~(\autoref{fig:boardwiring}).
Then it is connected via a ribbon cable to the original mouse PCB for wire-tapping (Stage~1) and to the new ADNS sensor board (Stage~2).
...
The pins from the XC are connected to the 10-pin header (left) and the measurement bridge (right)
}
\end{center}
\end{figure}
...
This is described in~\autoref{lb}
