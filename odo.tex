...
The goal of the experiments was to test the sensor on the required surfaces with speeds up to 5\,m/s at varying distances.
...
As reference speed sensor, a light barrier is used.
Reflective elements are mounted on the edge of the disk (see~\autoref{fig:refl}).
The light barrier is connected to the ADC on the microcontroller.\\



The light barrier is mounted 5\,mm away from the disk edge.
On the disk edge, eight reflective elements made of alumina foil with a length of 28\,mm are glued in even intervals.
Whenever a reflective part passes below the sensor, the output voltage of the photo diode changes.

As light barrier, a \emph{HOA1404 Reflective Sensor}~\cite{hoa} from \empf{Honeywell} is used.
It consists in an infra-red LED and a photo transistor.
